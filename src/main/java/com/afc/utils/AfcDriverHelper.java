...
public abstract class AfcDriverHelper 
{
	
public static final Logger logger = Logger.getLogger(AfcDriverHelper.class);
	
	// Declaring objects for FirstLifeDriverHelper class
	protected static WebDriver driver;
	public AfcPropertyReader propertyReader;
	...
	public AfcDriverHelper(WebDriver webdriver) {
		AfcDriverHelper.driver = webdriver;
		propertyReader = new AfcPropertyReader();
	...
	{
		for (int i = 0; i < timeout; i++) {
				if (!AfcDriverHelper.isElementPresent(locator)) {
					break;
				}
	...
	public void mouseOver(final String locator, final int timeout) {
		...
		final WebElement el = driver.findElement(AfcDriverHelper.byLocator(locator));
	...
	public void dragAndDrop(final String draggable, final String to, final int timeout) {
		...
		final WebElement elDraggable = driver.findElement(AfcDriverHelper.byLocator(draggable));
	...
	private Object _executeJavaScript(String jsCode) {
		return ((JavascriptExecutor) AfcDriverHelper.driver).executeScript(jsCode);
	}
	...
	private final ExpectedCondition<Boolean> jQueryLoad = new ExpectedCondition<Boolean>() {
		...
		public Boolean apply(WebDriver theDriver) {
			try {
				return ((Long) AfcDriverHelper.this._executeJavaScript("return jQuery.active") == 0);
			} catch (Exception e) {
				return true;
			}
		}
	...
	private final ExpectedCondition<Boolean> jsLoad = new ExpectedCondition<Boolean>() {
		@Override
		public Boolean apply(WebDriver theDriver) {
			Object rsltJs = AfcDriverHelper.this._executeJavaScript("return document.readyState");
			if (rsltJs == null) {
				rsltJs = "";
			}
	...
	private WebDriverWait _getWebDriverWait(int timeoutInSeconds) {
		return new WebDriverWait(AfcDriverHelper.driver, timeoutInSeconds);
	}
